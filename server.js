...
io.on('connection', (socket) => {
  ...
  socket.on('addSong', async ({ url, requester }) => {
    ...
    if (!info.embeddable) return socket.emit('error', '❌ This video cannot be embedded (copyright restriction)');

    const wasEmpty = queue.length === 0;
    queue.push({ id: info.id, title: info.title, artist: info.artist, requester: requester || "Skeleton Fan" });
